...
#     def get_all(state_abb):
#         rows = app.db.execute('''
# SELECT s.state_po, s.year, s.candidate, s.party_simplified, r.cand_ici, r.ttl_receipts, s.candidatevotes
# FROM Senate_Results s
# JOIN Receipts r 
# ON s.candidate LIKE r.cand_name
#     OR r.cvg_end_dt::VARCHAR LIKE s.year::VARCHAR
# WHERE s.state_po = :state_abb
# ''',
#                             state_abb = state_abb)
#         # print(state_abb)
#         return [State(*row) for row in rows]
...
''',
    ...
    @staticmethod
    def get_unique_years(state_abb): #return unique years
        rows = app.db.execute('''
WITH r AS (SELECT cand_office_st, SPLIT_PART(cvg_end_dt::VARCHAR(10), '-', 1) AS year, SPLIT_PART(cand_name, ',', 1) AS last_name, SPLIT_PART(cand_name, ' ', 2) AS first_name, cand_pty_affiliation, cand_ici, ttl_receipts
FROM Receipts),
s AS (SELECT SPLIT_PART(candidate, ' ', 1) AS first_name, SPLIT_PART(candidate, ' ', 2) AS last_name1, SPLIT_PART(candidate, ' ', 3) AS last_name2, candidate, year, (candidatevotes*100)/(totalvotes) AS percent_votes
...
''',
    ...
    @staticmethod
    def get_all_year(state_abb, year): #filtering by year
        rows = app.db.execute('''
WITH r AS (SELECT cand_office_st, SPLIT_PART(cvg_end_dt::VARCHAR(10), '-', 1) AS year, SPLIT_PART(cand_name, ',', 1) AS last_name, SPLIT_PART(cand_name, ' ', 2) AS first_name, cand_pty_affiliation, cand_ici, ttl_receipts
FROM Receipts),
s AS (SELECT SPLIT_PART(candidate, ' ', 1) AS first_name, SPLIT_PART(candidate, ' ', 2) AS last_name1, SPLIT_PART(candidate, ' ', 3) AS last_name2, candidate, year, (candidatevotes*100)/(totalvotes) AS percent_votes
...
#     @staticmethod
#     def get_years(state_abb): #list of all years
#         rows = app.db.execute('''
# WITH t AS 
# (WITH r AS (SELECT cand_office_st, SPLIT_PART(cvg_end_dt::VARCHAR(10), '-', 1) AS year, SPLIT_PART(cand_name, ',', 1) AS last_name, SPLIT_PART(cand_name, ' ', 2) AS first_name, cand_pty_affiliation, cand_ici, ttl_receipts
# FROM Receipts),
...
# AND s.year = :year
# ORDER BY s.year DESC)
# SELECT DISTINCT t.s.year
# FROM t
# ''',
#                             state_abb = state_abb)
#         return [Year(*row) for row in rows]